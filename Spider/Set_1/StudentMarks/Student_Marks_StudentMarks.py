...
class StudentMarks:
    ...
    def plotMarks(self,student_data,names_list):
        
        group_student_data=pd.DataFrame(student_data.groupby('Name').agg({'DS_Marks':'sum','Maths_Marks':'sum'}))
        print(group_student_data)
        
        plt.plot(group_student_data)
        plt.legend(['DS_Marks','Maths_Marks'])
        plt.grid(True)
...
def main():
    ...
    studentmarks.plotMarks(student_data,names_list)
